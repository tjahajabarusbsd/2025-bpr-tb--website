...
@section('title', 'BPR Tjahaja Baru | ' . $category->name)
...
@section('content')
  <div class="container mx-auto py-8 px-4">
    ...
    @if ($posts->count() == 0)
      <p class="text-gray-600">Belum ada informasi pada kategori ini.</p>
    @else
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($posts as $post)
          ...
          <a href="{{ route('post.show', [$post->category->slug, $post->slug]) }}"
            class="bg-white rounded-lg shadow hover:shadow-lg overflow-hidden group hover:scale-[1.03] transition duration-300">
            ...
            <div class="p-4">
              ...
              <p class="text-sm text-gray-600 line-clamp-3 mb-3">
                {!! Str::limit(
            strip_tags(
              preg_replace('/<figcaption[^>]*>.*?<\/figcaption>/is', '', $post->content)
            ),
            200
          ) !!}
              </p>
